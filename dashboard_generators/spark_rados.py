...
def basics():
    ...
    config['description'] = 'Dashboard showing CPU, Disk, and Network utilization of SkyhookDM'
    config['editable'] = True
    config['gnetId'] = None
    config['graphTooltip'] = 0
    config['id'] = 1
    config['iteration'] = 1619523594477
    config['links'] = []
    print(config)
    return config
...
def generate(reservation, outputloc):
    ...
    spark_nodes = [x for x in reservation.nodes if not 'designations' in x.extra_info]
